    ...
    public void render(AnalysisReport report) throws Exception {
        report.addNormalTitle("Administrative")
                .addParagraph(new Paragraph().setFontSize(FontSize.H5)
                        ...
                        .add(report.getDataSet().getAnalysisResult().getSummary().getToken())
                        .add(PdfUtils.createUrlLinkIcon(report.getDataSet().getIcon(),FontSize.H5, URL.ANALYSIS + report.getDataSet().getAnalysisResult().getSummary().getToken()))
